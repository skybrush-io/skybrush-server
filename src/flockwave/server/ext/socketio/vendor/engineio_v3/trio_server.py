...
class TrioQueue:
    ...
    def _ensure_join_event(self):
        if self._join_event is None:
            self._join_event = trio.Event()
            self._trigger_join_event_if_needed()

    def _trigger_join_event_if_needed(self):
        if self._join_event and self._tasks_pending == 0:
            self._join_event.set()
            self._join_event = None


def run_in_nursery(nursery, func, *args, **kwds):
    nursery.start_soon(partial(func, *args, **kwds))


class TrioServer(server.Server):
    """An Engine.IO server for Trio.
    ...
    def __init__(self, *args, nursery=None, **kwds):
        self.use_nursery(nursery)
        ...
        if sid is not None:
            ...
            async with trio.open_nursery() as nursery:
                for client in self.sockets.values():
                    nursery.start_soon(client.close)
            self.sockets = {}
    ...
    async def _handle_wsgi_request(self, environ):
        ...
        if jsonp and jsonp_index is None:
            ...
            if sid is None:
                ...
                if transport != "polling" and transport != "websocket":
                    ...
                    r = await self._handle_connect(environ, transport, b64, jsonp_index)
            else:
        ...
        if inspect.iscoroutinefunction(make_response):
            response = await make_response(
                r["status"], r["headers"] + cors_headers, r["response"], environ
            )
        else:
            response = make_response(
                r["status"], r["headers"] + cors_headers, r["response"], environ
    ...
    def start_background_task(self, target, *args, **kwargs):
        ...
        """
        self._nursery.start_soon(partial(target, *args, **kwargs))
    ...
    def create_queue(self, *args, **kwargs):
        ...
        async mode. For Trio based async modes, this returns an instance of
        ``TrioQueue``.
        """
        return TrioQueue(*args, **kwargs)
    ...
    def get_queue_empty_exception(self):
        ...
        selected async mode. For Trio based async modes, this returns an
        instance of ``trio.WouldBlock``.
        """
        return trio.WouldBlock
    ...
    def create_event(self, *args, **kwargs):
        ...
        selected async mode. For Trio based async modes, this returns
        an instance of ``trio.Event``.
        """
        return trio.Event(*args, **kwargs)
    ...
    async def _handle_connect(self, environ, transport, b64=False, jsonp_index=None):
        ...
        if transport == "websocket":
            ...
            try:
                return self._ok(
                    await s.poll(), headers=headers, b64=b64, jsonp_index=jsonp_index
                )
