...
def get_carchive_info(filepath):
    ...
    magic, lengthofPackage, toc, tocLen, pyver, pylibname = struct.unpack(
        "!8siiii64s", buf[pos : pos + PYINST_COOKIE_SIZE]
    )
    fp.close()
...
def append_runtime_files(logic_toc, obfpath):
    ...
    def add_toc(typcd, name, pathnm):
        ...
        if os.path.isdir(pathnm):
            raise RuntimeError(
                'It is not allowed to write path "%s" to '
        ...
        if n > 1:
            raise RuntimeError(
                'In the path "%s", there are too many '
    ...
    for name in os.listdir(obfpath):
        ...
        if (name.startswith("pytransform") and name[-3:] != ".py") or name.startswith(
            "_pytransform"
        ):
            n += 1
...
def main():
    ...
    parser.add_argument(
        "-e", "--entry", help="Entry script if it's different from bundle name"
    )
    parser.add_argument("executable", metavar="executable", help="PyInstaller archive")
