...
from ..engineio_v3.trio_server import TrioServer as EngineIOTrioServer

from . import trio_manager
...
class TrioServer(server.Server):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if client_manager is None:
            client_manager = trio_manager.TrioManager()
        super().__init__(
    ...
    async def call(
        self, event, data=None, to=None, sid=None, namespace=None, timeout=60, **kwargs
    ):
    ...
    async def disconnect(self, sid, namespace=None):
        ...
        if self.manager.is_connected(sid, namespace=namespace):
            ...
            await self._send_packet(
                sid, packet.Packet(packet.DISCONNECT, namespace=namespace)
            )
    ...
    async def _handle_wsgi_request(self, *args, **kwargs):
        """Handle a WSGI HTTP request from the client.

        This is handy if we have a wrapper that has already translated the
        incoming ASGI request to WSGI.
        """
        return await self.eio._handle_wsgi_request(*args, **kwargs)
    ...
    async def _handle_connect(self, sid, namespace):
        ...
        if self.always_connect:
            await self._send_packet(
                sid, packet.Packet(packet.CONNECT, namespace=namespace)
            )
        fail_reason = None
        try:
            success = await self._trigger_event(
                "connect", namespace, sid, self.environ[sid]
            )
        ...
        if success is False:
            ...
            if not self.always_connect:
                await self._send_packet(
                    sid,
                    packet.Packet(packet.ERROR, data=fail_reason, namespace=namespace),
                )
    ...
    async def _handle_event_internal(self, server, sid, data, namespace, id):
        ...
        if id is not None:
            ...
            await server._send_packet(
                sid,
                packet.Packet(
                    packet.ACK, namespace=namespace, id=id, data=data, binary=None
                ),
            )
    ...
    async def _trigger_event(self, event, namespace, *args):
        ...
        if namespace in self.handlers and event in self.handlers[namespace]:
            if inspect.iscoroutinefunction(self.handlers[namespace][event]) is True:
                ret = None
